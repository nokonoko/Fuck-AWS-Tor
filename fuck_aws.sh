...
select yn in "Yes" "No"; do
    case $yn in
        ...
        No ) wget https://ip-ranges.amazonaws.com/ip-ranges.json && cat ip-ranges.json | jq --raw-output '.prefixes[] | .ip_prefix' > list.txt && rm ip-ranges.json && echo "Complete!! Saved as list.txt"; break;;
    esac
done
